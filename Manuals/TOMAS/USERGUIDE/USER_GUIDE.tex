...
\centering
\includegraphics[width=\linewidth]{Glow1}
\captionsetup{width=0.75\textwidth}
\captionof{figure}{The Glow discharge power control}
\label{Glow1}
\end{minipage}

\includegraphics[width=\linewidth]{Glow2}
\captionsetup{width=0.75\textwidth}
\captionof{figure}{The Glow discharge power control}
\label{Glow2}
\vspace{0.2cm}

Once the plasma discharge has initiated, you can change the value for the gas flow and/or the current. Don't do this abruptly so that the discharge will remain. The value for the current should not exceed 5 A. When you change the current, the voltage will change. For a hydrogen gas with a gas flow of 150 ml/min, we obtain the following values:\\

\begin{minipage}{.3\textwidth}
\begin{tabular}{|c|c|c|c|c|c|}
	\hline
	V & 314 & 338 & 357 & 375 & 392 \\ \hline
	A &  1 & 2 & 3 & 4  & 5 \\ \hline
\end{tabular}
\end{minipage}
\begin{minipage}{.02\textwidth}
...
\textcolor{red}{\textbf{Change the value for the gas flow and/or the current as desired.}}\\
\end{minipage}
\vspace*{0.4cm}

When there is no initial plasma discharge, increase the gas pressure by closing the valve to the turbopomp for a short period of time ($\approx 5 s).
...
\subsection{EC matching procedure}\label{ECmatching}
